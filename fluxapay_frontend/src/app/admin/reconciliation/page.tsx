        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500 font-medium">
                Verify USDC swept ≈ Total Fiat Payouts + Fees · Detect
          ...
          <div className="mt-4 flex gap-1 bg-slate-100 rounded-xl p-1 w-fit">
            {[
              ...
              {
                key: "alerts" as const,
                label: `Alerts ${unresolvedAlerts.length > 0 ? `(${unresolvedAlerts.length})` : ""}`,
                icon: Bell,
              },
            ].map((tab) => (
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
              {[
                ...
                },
                {
                  ...
                  icon: Percent,
                  color:
                    stats.avgDiscrepancy > 1
                      ? "text-amber-600"
                      : "text-emerald-600",
                  bg:
                    stats.avgDiscrepancy > 1 ? "bg-amber-50" : "bg-emerald-50",
                  sub:
                    stats.avgDiscrepancy <= 1
                      ? "Within threshold"
                      : "Above 1% threshold",
                ...
                <div className="mt-8 bg-white/5 rounded-2xl p-4 border border-white/10">
                  <div className="flex justify-between text-xs font-bold mb-2">
                    <span className="text-slate-400">
                      Balance Match Progress
                    </span>
                    <span className="text-emerald-400">
                      {(
                        (stats.matchedCount / Math.max(stats.totalRecords, 1)) *
                        100
                      ).toFixed(0)}
                      % matched
                    </span>
                  ...
                  <div className="h-2 rounded-full bg-white/10 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(stats.matchedCount / Math.max(stats.totalRecords, 1)) * 100}%`,
                      }}
                    />
                  </div>
                </div>
              ...
              {allAlerts.length === 0 ? (
                ...
                allAlerts.map((alert) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`p-2.5 rounded-xl ${sevConfig.bg} border ${sevConfig.border} flex-shrink-0`}
                      >
                        <AlertTriangle
                          className={`w-5 h-5 ${sevConfig.color}`}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${sevConfig.bg} ${sevConfig.color} border ${sevConfig.border}`}
                          >
                            <span
                          ...
                          <span className="text-[10px] text-slate-400">
                            {"reconciliationId" in alert &&
                              (alert as unknown as { reconciliationId: string })
                                .reconciliationId}
                          </span>
                        </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {(() => {
                      ...
                      return (
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${cfg.bg} ${cfg.color} border ${cfg.border}`}
                        >
                          {cfg.icon}
                          {cfg.label}
                        </span>
              ...
              selectedRecord.status === "discrepancy" ? (
                <>
                  <button
                    onClick={() => {
                      toast.success(
                        `Reconciliation ${selectedRecord.id} marked as reviewed`,
                      );
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs text-amber-800 font-medium">
                    This will compare total USDC swept against total fiat
                    payouts + fees for the selected period. Alerts will be
                    generated for discrepancies above 1%.
                  </p>
                </div>
              </div>
            </div>
